...
static void render_callback(Canvas* canvas, void* ctx) {
    ...
    snprintf(scount, sizeof(scount), "%d", c->counts[c->count_index]);
...
int32_t counterapp(void) {
    ...
    while(furi_message_queue_get(c->input_queue, &input, FuriWaitForever) == FuriStatusOk) {
        ...
        case InputKeyBack:
            ...
            return 0;
        case InputKeyUp:
            if(c->counts[c->count_index] < MAX_COUNT) {
                c->counts[c->count_index]++;
                c->pressed = true;
                c->boxtimer = BOXTIME;
            }
            break;
        case InputKeyDown:
            if(c->counts[c->count_index] > 0) {
                c->counts[c->count_index]--;
                c->pressed = true;
                c->boxtimer = BOXTIME;
            }
            break;
        case InputKeyRight:
            if(c->count_index < NUM_COUNTERS - 1) {
                c->count_index++;
            } else {
                c->count_index = 0;
            }
